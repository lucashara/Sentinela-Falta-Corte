/*-----------------------------------------------------------
   ...
   PERÍODO :DATAI–:DATAF  VS.  BENCHMARK TRIMESTRAL (ÚLTIMOS 91 DIAS)
-----------------------------------------------------------*/
WITH PARAM AS (SELECT :DATAI                             AS DATAI,
     ...
     MEDIA_PERIODO_CORTE AS (SELECT CODFILIAL,
                                    ROUND(AVG(PCT_CORTE), 2) AS MEDIA_PERIODO_CORTE
     ...
     MEDIA_PERIODO_FALTA AS (SELECT CODFILIAL,
                                    ROUND(AVG(PCT_FALTA), 2) AS MEDIA_PERIODO_FALTA
                             FROM DIARIO_PERIODO_FALTA
                             GROUP BY CODFILIAL),

     DIARIO_TRIM_CORTE AS (SELECT CODFILIAL,
                                  DATA,
                                  CASE
                                      WHEN SUM(VLVENDA) = 0 THEN 0
                                      ELSE ROUND(SUM(QTCORTADA * PVENDA) / SUM(VLVENDA) * 100, 2)
     ...
     MEDIA_TRIM_CORTE AS (SELECT CODFILIAL,
                                 ROUND(AVG(PCT_CORTE), 2) AS MEDIA_TRIM_CORTE
                          FROM DIARIO_TRIM_CORTE
                          GROUP BY CODFILIAL),
     ...
     MEDIA_TRIM_FALTA AS (SELECT CODFILIAL,
                                 ROUND(AVG(PCT_FALTA), 2) AS MEDIA_TRIM_FALTA
                            ...
                            ROUND(
                                    NVL((SELECT SUM(QTCORTADA * PVENDA)
                                         FROM PCCORTEI,
                                              PARAM
                                         WHERE DATA BETWEEN PARAM.DATAI AND PARAM.DATAF
                                           AND CODFILIAL = FILIAIS.CODFILIAL), 0)
                                , 2) AS PVENDA_CORTE
                     FROM FILIAIS),
     ...
     RESULT AS (
         ...
         SELECT DADOS_CORTE.CODFILIAL,
                ...
                TO_CHAR(MEDIA_TRIM_CORTE.MEDIA_TRIM_CORTE, 'FM990D00') || '%'       AS MEDIA_TRIM_CORTE,
                CASE
                    WHEN MEDIA_PERIODO_CORTE.MEDIA_PERIODO_CORTE > MEDIA_TRIM_CORTE.MEDIA_TRIM_CORTE
                        THEN '+' || TO_CHAR(MEDIA_PERIODO_CORTE.MEDIA_PERIODO_CORTE - MEDIA_TRIM_CORTE.MEDIA_TRIM_CORTE,
                                            'FM990D00') || '% ACIMA'
                    WHEN MEDIA_PERIODO_CORTE.MEDIA_PERIODO_CORTE < MEDIA_TRIM_CORTE.MEDIA_TRIM_CORTE
                        THEN '-' || TO_CHAR(MEDIA_TRIM_CORTE.MEDIA_TRIM_CORTE - MEDIA_PERIODO_CORTE.MEDIA_PERIODO_CORTE,
                                            'FM990D00') || '% ABAIXO'
                    ELSE '0% (NA MÉDIA)'
                    END                                                             AS DESVIO_CORTE,
         ...
         FROM DADOS_CORTE
                  ...
                  JOIN MEDIA_TRIM_CORTE ON DADOS_CORTE.CODFILIAL = MEDIA_TRIM_CORTE.CODFILIAL
         ...
         SELECT 'TOTAL'                                                                      AS CODFILIAL,
                ...
                TO_CHAR(ROUND(SUM(DADOS_CORTE.PVENDA_CORTE) / NULLIF(SUM(DADOS_FATURAMENTO.FATURAMENTO), 0) * 100, 2),
                        'FM990D00') || '%'                                                   AS PCT_PERIODO_CORTE,
                TO_CHAR(ROUND(AVG(MEDIA_TRIM_CORTE.MEDIA_TRIM_CORTE), 2), 'FM990D00') || '%' AS MEDIA_TRIM_CORTE,
                CASE
                    WHEN ROUND(SUM(DADOS_CORTE.PVENDA_CORTE) / NULLIF(SUM(DADOS_FATURAMENTO.FATURAMENTO), 0) * 100, 2)
                        > AVG(MEDIA_TRIM_CORTE.MEDIA_TRIM_CORTE)
                        THEN '+' || TO_CHAR(
                ...
                TO_CHAR(ROUND(SUM(DADOS_FALTA.PVENDA_FALTA) / NULLIF(SUM(DADOS_FATURAMENTO.FATURAMENTO), 0) * 100, 2),
